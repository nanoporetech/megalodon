...
def plot_calib(
    ...
    if add_prob_thresh:
        ...
        for p, col in zip(prob_threshs, PROB_COLORS):
            ...
            prop_filt = (
                sum(sm_ref[nthresh_val:thresh_val])
            ...
            axarr[2].axvline(
                x=smooth_ls[nthresh_val],
                color=col,
                label=("--mod-binary-threshold={} (filters {:.0f}%)").format(
                    p, 100 * prop_filt
                ),
            )
...
def _main(args):
    ...
    for mod_base, (mod_llrs, can_llrs) in mod_base_llrs.items():
        LOGGER.info("Computing {} modified base calibration.".format(mod_base))
