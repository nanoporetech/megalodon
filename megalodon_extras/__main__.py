...
NESTED_COMMANDS = [
    (
        ...
        [
            (ep.CMD_AGG_RUN, "Run aggregation"),
        ],
    ),
    (
        ep.GRP_CALIB,
    ...
    (
        ...
        [
            (ep.CMD_MERGE_MODS, "Merge per-read modified base database"),
            (
                ep.CMD_MERGE_AGG_MODS,
                "Merge aggregated modified base bedmethyl files",
            ),
            (ep.CMD_MERGE_VARS, "Merge per-read sequence variants database"),
        ],
    ),
    (
        ep.GRP_MODS,
        ...
        [
            (ep.CMD_MODS_ALPHABET, "Print the alphabet for a choosen model"),
            (
            ...
            (ep.CMD_MODS_MOTIF, "Create BED file of motif sites"),
            (
                ep.CMD_MODS_PER_SITE,
                "Extract per-site modified base thresholds for signal mapping "
                + "sequence markup",
            ),
            (ep.CMD_MODS_INDEX, "Create per-read modified base database index"),
            (ep.CMD_MODS_SPLIT, "Split modified base database by motif"),
        ],
    ),
    (
        ep.GRP_PHASE,
        ...
        [
            ...
            (ep.CMD_PHASE_MERGE_HAP, "Merge variants from haploid calls"),
        ],
    ),
    (
        ep.GRP_TXT,
    ...
    (
        ...
        [
            ...
            (ep.CMD_VAL_AGG_MODS, "Validate aggregated modified bases results"),
            (
    ...
    (
        ...
        [
            (ep.CMD_VAR_ATOM, "Atomize variants for faster processing"),
            (ep.CMD_VAR_RESOLVE, "Resolve potentially conflicting variants"),
            (
            ...
            # database exists.
            (ep.CMD_VAR_INDEX, "***** Stub for future implementation *****"),
        ],
    ),
]
...
def _main():
    """The main routine."""
    desc = (
        "Megalodon extras command groups (additional help available "
    ...
    module = import_module(
        ".{}_{}".format(args.service_command, args.action_command),
        "megalodon_extras",
    )
    module._main(args)
