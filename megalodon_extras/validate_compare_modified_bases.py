...
def get_coverage_plot_data(
    ...
    cov_start = min(
        np.searchsorted(samp1_pctl, trim_lower_pct, side="left"),
        np.searchsorted(samp2_pctl, trim_lower_pct, side="left"),
    )
    cov_end = max(
        np.searchsorted(samp1_pctl, trim_upper_pctl, side="right"),
    ...
    return (
        np.arange(cov_start, cov_end),
        samp1_bin_cov[cov_start:cov_end],
...
def plot_cov(
    ...
    plt.figure(figsize=(6, 5))
    plt.bar(
        x_ind, samp1_bin_cov, width, color=COV_SAMP1_COLOR, label=samp_names[0]
    )
    plt.bar(
        x_ind + width,
        samp2_bin_cov,
...
def plot_hm(
    samp1_mod_pct, samp2_mod_pct, hm_num_bins, samp_names, out_fp, pdf_fp
):
    ...
    for mod_pct in (0, 25, 50, 75, 100):
        hm_ticks[int((hm_num_bins - 1) * mod_pct / 100)] = str(mod_pct)
    ...
    hm = sns.heatmap(
        log_hist_data,
        vmin=max(log_hist_data.min(), 1),
        vmax=log_hist_data.max(),
        cmap=CMAP,
        square=True,
        xticklabels=hm_ticks,
        yticklabels=hm_ticks[::-1],
    )
...
def compute_filt_mod_pct(
    ...
    if len(common_ctgs) == 0:
        samp1_ctg_str = ",".join(
            [f"{chrm}:{strand}" for chrm, strand in sorted(samp1_cov)[:5]]
        )
        samp2_ctg_str = ",".join(
            [f"{chrm}:{strand}" for chrm, strand in sorted(samp2_cov)[:5]]
        )
        LOGGER.error(
            "No common contigs found between provided samples.\n"
            f'\t"{samp_names[0]}" first 5 contigs: {samp1_ctg_str}\n'
            f'\t"{samp_names[1]}" first 5 contigs: {samp2_ctg_str}'
        )
        sys.exit(1)
    ...
    for ctg in common_ctgs:
        for pos in set(samp1_cov[ctg]).intersection(samp2_cov[ctg]):
            ...
            if min(samp1_pos_cov, samp2_pos_cov) < cov_thresh:
                continue
            samp1_mod_pct.append(100 * samp1_mod_cov[ctg][pos] / samp1_pos_cov)
            samp2_mod_pct.append(100 * samp2_mod_cov[ctg][pos] / samp2_pos_cov)
    ...
    out_fp.write(
        "{} valid coverage median: {:.2f}   mean: {:.2f}  sd: {:.2f}\n".format(
            samp_names[0],
            np.median(samp1_valid_cov),
            np.mean(samp1_valid_cov),
            np.std(samp1_valid_cov),
        )
    )
    out_fp.write(
        "{} valid coverage median: {:.2f}   mean: {:.2f}  sd: {:.2f}\n".format(
            samp_names[1],
            np.median(samp2_valid_cov),
            np.mean(samp2_valid_cov),
            np.std(samp2_valid_cov),
        )
    )
    out_fp.write(
        "{} valid corresponding sites detected\n".format(len(samp1_mod_pct))
    )

    return (
        np.array(samp1_mod_pct),
        np.array(samp2_mod_pct),
        np.array(samp1_valid_cov),
        np.array(samp2_valid_cov),
    )


def parse_inputs(
    samp1_bm_fns, samp2_bm_fns, strand_offset, samp_names, valid_pos_fn, out_fp
):
...
def _main(args):
    ...
    (
        ...
        args.sample1_bed_methyl_files,
        args.sample2_bed_methyl_files,
        args.strand_offset,
        args.sample_names,
        args.valid_positions,
        out_fp,
    )
    (
        samp1_mod_pct,
