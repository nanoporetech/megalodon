...
def call_variant(
    ...
    var_context_bases = (
        context_bases[0]
        if len(var_ref_seq) == len(var_alt_seq)
        else context_bases[1]
    )
    ...
    if ref_seq is None:
        ...
        pos_ref_seq = np_ref_seq[
            r_var_pos - pos_bb : r_var_pos + pos_ab + len(var_ref_seq)
        ]
    else:
        pos_ab = min(
            var_context_bases, len(ref_seq) - r_var_pos - len(var_ref_seq)
        )
        pos_ref_seq = mh.seq_to_int(
            ref_seq[r_var_pos - pos_bb : r_var_pos + pos_ab + len(var_ref_seq)]
        )

    pos_alt_seq = np.concatenate(
        [
            pos_ref_seq[:pos_bb],
            mh.seq_to_int(var_alt_seq),
...
def call_alt_true_indel(
    ...
    if (
        r_algn.r_st > r_var_pos - context_bases[1]
        or r_algn.r_en < r_var_pos + context_bases[1]
    ):
        raise mh.MegaError("Indel mapped read clipped variant position.")

    post_mapped_start = rl_cumsum[r_algn.q_st]
    mapped_rl_cumsum = (
        rl_cumsum[r_algn.q_st : r_algn.q_en + 1] - post_mapped_start
    )
...
def process_read(
    ...
    var_poss = list(
        range(edge_buffer, np_ref_seq.shape[0] - edge_buffer, every_n)
    )[:max_pos_per_read]
    read_var_calls = []
        ...
        if (
            len(
                set(
                    r_ref_seq[
                        r_var_pos
                        - max(context_bases) : r_var_pos
                        + max_indel_len
                        + 1
                        + max(context_bases)
                    ]
                ).difference(CAN_BASES_SET)
            )
            > 0
        ):
...
def _get_variant_calls(
    ...
    if len(err_types) > 0:
        ...
        for n_errs, err_str in sorted((v, k) for k, v in err_types.items())[
            ::-1
        ]:
...
def process_all_reads(
    ...
    files_p = mp.Process(
        ...
        args=(input_info, fn_read_ids_q, num_reads_conn, aux_failed_q),
        daemon=True,
        name="FileFiller",
    )
    files_p.start()
    ...
    for espi in range(num_extract_sig_ps):
        extract_sig_ps.append(
            mp.Process(
                ...
                daemon=True,
                name="ExtractSig{:03d}".format(espi),
            )
        )
    ...
    for bcpi, device in enumerate(model_info.process_devices):
        bc_ps.append(
            mp.Process(
                target=basecall_worker,
                args=(sig_q, bc_q, model_info, device),
                daemon=True,
                name="Basecaller{:03d}".format(bcpi),
            )
        )
        bc_ps[-1].start()

    var_calls_q, var_calls_p, main_sc_conn = mega_mp.create_getter_qpc(
        _get_variant_calls, (out_fn, getter_num_reads_conn, suppress_progress)
    )
