    ...
    with open(llr_fn) as llr_fp:
        for line in llr_fp:
            ...
            if np.isnan(llr):
                continue
            if (
                max_indel_len is not None
                and np.abs(len(ref_seq) - len(alt_seq)) > max_indel_len
            ):
                continue
...
def _main(args):
    ...
    for (ref_seq, alt_seq), snp_llrs in sorted(snp_ref_llrs.items()):
        LOGGER.info(
            "Computing " + ref_seq + " -> " + alt_seq + " SNP calibration."
        )
        try:
            (
                snp_calib,
                ...
                np.array(snp_llrs),
                args.max_input_llr,
                args.num_calibration_values,
                args.smooth_bandwidth,
                args.min_density,
                args.diff_epsilon,
                args.llr_clip_buffer,
                pdf_fp is not None,
    ...
    for del_len, del_llrs in sorted(del_ref_llrs.items()):
        LOGGER.info("Computing deletion length {} calibration.".format(del_len))
        try:
            (
                del_calib,
                del_llr_range,
    ...
    for ins_len, ins_llrs in sorted(ins_ref_llrs.items()):
        LOGGER.info(
            "Computing insertion length {} calibration.".format(ins_len)
        )
        try:
            (
                ins_calib,
    ...
    for (ref_seq, alt_seq), (snp_calib, snp_llr_range) in snp_calibs.items():
        snp_calib_save_data[
            calibration.SNP_CALIB_TMPLT.format(ref_seq, alt_seq)
        ] = snp_calib
        snp_llr_range_save_data[
            calibration.SNP_LLR_RNG_TMPLT.format(ref_seq, alt_seq)
        ] = snp_llr_range
    del_llr_range_save_data, del_calib_save_data = {}, {}
    for del_len, (del_calib, del_llr_range) in del_calibs.items():
        del_calib_save_data[
            calibration.DEL_CALIB_TMPLT.format(del_len)
        ] = del_calib
        del_llr_range_save_data[
            calibration.DEL_LLR_RNG_TMPLT.format(del_len)
        ] = del_llr_range
    ins_llr_range_save_data, ins_calib_save_data = {}, {}
    for ins_len, (ins_calib, ins_llr_range) in ins_calibs.items():
        ins_calib_save_data[
            calibration.INS_CALIB_TMPLT.format(ins_len)
        ] = ins_calib
        ins_llr_range_save_data[
            calibration.INS_LLR_RNG_TMPLT.format(ins_len)
        ] = ins_llr_range
    np.savez(
        args.out_filename,
