...
def get_parser_aggregate_run():
    ...
    misc_grp.add_argument(
        ...
        help="Batch size for aggregation processing. Default: %(default)d",
    )
    misc_grp.add_argument(
        "--processes",
        type=int,
...
def get_parser_calibrate_modified_bases():
    ...
    parser.add_argument(
        "--out-filename",
        default="megalodon_mod_calibration.npz",
...
def get_parser_calibrate_merge_modified_bases():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "modified_base_calibration_files",
        nargs="+",
...
def get_parser_calibrate_merge_modified_bases_stats():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "modified_base_calibration_stats_files",
        nargs="+",
...
def get_parser_calibrate_variants():
    ...
    parser.add_argument(
        "--out-filename",
        default="megalodon_variant_calibration.npz",
        help="Filename to output calibration values. Default: %(default)s",
    )
    parser.add_argument(
        "--out-pdf",
...
def get_parser_calibrate_generate_mod_stats_from_msf():
    ...
    pyg_grp.add_argument(
        "--guppy-config",
        default=mh.DEFAULT_GUPPY_CFG,
        help="Guppy config. Default: %(default)s",
    )
    pyg_grp.add_argument(
        "--guppy-server-path",
        default=mh.DEFAULT_GUPPY_SERVER_PATH,
    ...
    out_grp.add_argument(
        "--log-filename",
        default="mod_calibration_statistics.log",
        help="Output filename for text summary. Default: %(default)s",
    )
    out_grp.add_argument(
        "--modified-bases-set",
...
def get_parser_merge_aggregated_modified_bases():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "bed_methyl_files", nargs="+", help="Input bedmethyl format files."
    )
    parser.add_argument(
...
def get_parser_modified_bases_describe_alphabet():
    ...
    f5_grp.add_argument("--fast5s_dir", help="Directory containing raw fast5.")
...
def get_parser_modified_bases_update_database():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "old_db", help="Megalodon version 0.1 modified base data base."
    )
    parser.add_argument(
        "--new-db",
...
def get_parser_modified_bases_create_ground_truth():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--bed-methyl-files", nargs="+", required=True, help="Bed methyl files."
    )
    parser.add_argument(
        "--coverage-threshold",
        type=int,
    ...
    parser.add_argument(
        "--out-csv",
        default="ground_truth_modifications.csv",
...
def get_parser_modified_bases_per_site_thresholds():
    ...
    parser.add_argument(
        "--log-filename",
        default="per_site_thresholds.log",
...
def get_parser_phase_variants_extract_haplotype_reads():
    parser = argparse.ArgumentParser()
    parser.add_argument("alignment_filename", help="Alignment filename.")
    parser.add_argument("out_basename", help="Basename for read ids output.")
...
def get_parser_phase_variants_merge_haploid_variants():
    ...
    parser.add_argument(
        "haplotype1_variants", help="Variant file for haplotype 1."
    )
    parser.add_argument(
        "haplotype2_variants", help="Variant file for haplotype 2."
    )
    parser.add_argument(
        "--out-vcf",
        default="merged_haploid_variants.vcf",
...
def get_parser_validate_results():
    ...
    out_grp.add_argument(
        "--out-pdf",
        default="megalodon_validation.pdf",
...
def get_parser_validate_aggregated_modified_bases():
    ...
    parser.add_argument(
        "--out-pdf",
        default="megalodon_agg_validation.pdf",
        help="Output pdf filename. Default: %(default)s",
    )
    parser.add_argument(
        "--out-filename",
...
def get_parser_variants_atomize():
    ...
    parser.add_argument(
        "--out-vcf",
        default="atomized_variants.megalodon.vcf",
...
def get_parser_variants_resolve():
    ...
    parser.add_argument(
        "--output-filename",
        default="megalodon.consolidated_variants.vcf",
...
def get_parser_variants_heterozygous_factor():
    ...
    parser.add_argument(
        "megalodon_variants",
        default="megalodon_results/variants.vcf",
...
PARSERS = {
    GRP_AGG: {CMD_AGG_RUN: get_parser_aggregate_run},
    GRP_CALIB: {
        ...
        CMD_CALIB_GEN_MODS_MSF: get_parser_calibrate_generate_mod_stats_from_msf,
        ...
        CMD_CALIB_MERGE_MODS_STATS: get_parser_calibrate_merge_modified_bases_stats,
    },
    GRP_MERGE: {
    ...
    GRP_PHASE: {
        ...
        CMD_PHASE_GET_HAP_READS: get_parser_phase_variants_extract_haplotype_reads,
