...
CONTEXT_BASE_MI_LINE = (
    ...
    + '"REF and ALT contain a single upstream context base">'
).format(HAS_CONTEXT_BASE_TAG)
FIXED_VCF_MI = [
    "phasing=none",
...
class VarsDb:
    ...
    db_tables = OrderedDict(
        (
            ...
            (
                "alt",
                OrderedDict(
                    (("alt_id", "INTEGER PRIMARY KEY"), ("alt_seq", "TEXT"))
                ),
            ),
            (
                "read",
    ...
    def __init__(
        ...
        if self.read_only:
            ...
            self.cur.execute(
                "PRAGMA mmap_size = {}".format(mh.MEMORY_MAP_LIMIT)
            )
        ...
        try:
            if self.chrm_idx_in_mem:
                ...
                chrm_id = self.cur.execute(
                    "SELECT chrm_id FROM chrm WHERE chrm=?", (chrm,)
                ).fetchone()[0]
        except (TypeError, KeyError):
            self.cur.execute(
                "INSERT INTO chrm (chrm, chrm_len) VALUES (?,?)",
                (chrm, chrm_len),
            )
    ...
    def insert_chrms(self, ref_names_and_lens):
        ...
        if self.chrm_idx_in_mem:
            self.chrm_id_idx.update(
                zip(
                    ref_names_and_lens[0],
                    range(
                        next_chrm_id, next_chrm_id + len(ref_names_and_lens[0])
                    ),
                )
            )
            self.chrm_name_idx.update(
                zip(
                    range(
                        next_chrm_id, next_chrm_id + len(ref_names_and_lens[0])
                    ),
    ...
    def get_loc_ids_or_insert(self, r_var_scores, chrm_id):
        ...
        r_locs = dict(
            (
                ...
                for pos, _, ref_seq, _, var_name, test_start, test_end in r_var_scores
            )
        )
        if self.loc_idx_in_mem:
                ...
                (
                    ...
                    for test_start, test_end, loc_id in self.cur.execute(
                        (
                            ...
                            + "test_start in ({0}) AND test_end in ({1})"
                        ).format(
                            ",".join(
                                [
                                    "?",
        ...
        try:
            if self.alt_idx_in_mem:
                ...
                alt_id = self.cur.execute(
                    "SELECT alt_id FROM alt WHERE alt_seq=?", (alt_seq,)
                ).fetchone()[0]
        except (TypeError, KeyError):
            self.cur.execute("INSERT INTO alt (alt_seq) VALUES (?)", (alt_seq,))
    ...
    def get_alt_ids_or_insert(self, r_var_scores):
        ...
        r_uniq_seqs = set(
            alt_seq
            for _, _, _, alt_seqs, _, _, _ in r_var_scores
        ...
        try:
            if self.uuid_idx_in_mem:
                ...
                read_id = self.cur.execute(
                    "SELECT read_id FROM read WHERE uuid=?", (uuid,)
                ).fetchone()[0]
        except (TypeError, KeyError):
    ...
    def insert_read_scores(self, r_var_scores, uuid, chrm, strand):
        self.cur.execute(
            "INSERT INTO read (uuid, strand) VALUES (?,?)", (uuid, strand)
        )
        if len(r_var_scores) == 0:
            return
    ...
    def load_chrm_index(self):
        self.chrm_name_idx = dict(
            self.cur.execute("SELECT chrm_id, chrm FROM chrm").fetchall()
        )
    ...
    def load_uuid_read_index(self):
        ...
        self.uuid_read_idx = dict((read_id, uuid) for read_id, uuid in self.cur)
    ...
    def create_loc_index(self):
        self.cur.execute(
            "CREATE UNIQUE INDEX loc_idx ON loc"
    ...
    def create_data_covering_index(self):
        self.cur.execute(
            "CREATE INDEX data_cov_idx ON data("
    ...
    def get_chrm_id(self, chrm):
        try:
            ...
            raise mh.MegaError(
                "Reference record (chromosome) not found in " + "database."
            )
        return chrm_id
        ...
        try:
            if self.chrm_idx_in_mem:
                ...
                chrm = self.cur.execute(
                    "SELECT chrm FROM chrm WHERE chrm_id=?", (chrm_id,)
                ).fetchone()[0]
        except (TypeError, KeyError):
            raise mh.MegaError(
                "Reference record (chromosome) not found in " + "vars database."
            )
        return chrm
        ...
        try:
            if self.alt_idx_in_mem:
                ...
                alt_seq = self.cur.execute(
                    "SELECT alt_seq FROM alt WHERE alt_id=?", (alt_id,)
                ).fetchone()[0]
        except (TypeError, KeyError):
        ...
        try:
            if self.uuid_idx_in_mem:
                ...
                uuid = self.cur.execute(
                    "SELECT uuid FROM read WHERE read_id=?", (read_id,)
                ).fetchone()[0]
        except (TypeError, KeyError):
        ...
        try:
            if self.uuid_strand_idx_in_mem:
                ...
                uuid_strand = self.cur.execute(
                    "SELECT uuid, strand FROM read WHERE read_id=?", (read_id,)
                ).fetchone()
        except (TypeError, KeyError):
            raise mh.MegaError("Read ID not found in vars database.")
        return uuid_strand
    ...
    def get_num_uniq_stats(self):
        num_stats = self.cur.execute("SELECT MAX(rowid) FROM data").fetchone()[
            0
        ]
...
def score_seq(tpost, seq, tpost_start=0, tpost_end=None, all_paths=False):
...
def score_variants_with_context(
    ...
    for (
        ...
        max_var_len = max(
            np_s_var_ref_seq.shape[0],
            max(var_alt_seq.shape[0] for var_alt_seq in np_s_var_alt_seqs),
        )
        filt_np_s_context_seqs = [
            (up_seq, dn_seq)
            for up_seq, dn_seq in np_s_context_seqs
        ...
        for np_s_var_alt_seq, var_alt_seq, ref_cntxt_alt_lp in zip(
            ...
            # calibrate log probs
            loc_alt_llrs.append(
                calibrate_llr(loc_ref_lp - loc_alt_lp, variant.ref, var_alt_seq)
            )
...
def score_variants_independently(
    ...
    for (
        ...
        if blk_end - blk_start <= max(
            len(up_seq) + len(dn_seq) for up_seq, dn_seq in np_s_context_seqs
        ) + max(
            np_s_var_ref_seq.shape[0],
            max(var_alt_seq.shape[0] for var_alt_seq in np_s_var_alt_seqs),
        ):
...
def call_read_vars(
    ...
    read_ref_fwd_seq = (
        strand_read_np_ref_seq
        if read_ref_pos.strand == 1
        else mh.revcomp_np(strand_read_np_ref_seq)
    )
...
def score_all_single_deletions(
    ...
    for del_pos in range(start, end):
        ...
        np_alts = [
            np.array([], dtype=np.uintp),
        ]
        ctxt_seqs = [
            [
                read_np_seq[ctxt_start:del_pos],
    ...
    return [
        ...
        for pos, scores, _, _, _, _, _ in score_variants_independently(
            vars_iter, ref_to_block, r_post, read_np_seq
        )[0]
    ]
...
def score_all_single_insertions(
    ...
    return [
        (score, pos, alt)
        for pos, scores, _, alts, _, _, _ in score_variants_independently(
            vars_iter, ref_to_block, r_post, read_np_seq
        )[0]
...
def simplify_var_seq(ref_seq, alt_seq):
    ...
    while len(ref_seq) > 0 and len(alt_seq) > 0 and ref_seq[0] == alt_seq[0]:
        trim_before += 1
        ref_seq = ref_seq[1:]
        alt_seq = alt_seq[1:]
    while len(ref_seq) > 0 and len(alt_seq) > 0 and ref_seq[-1] == alt_seq[-1]:
        trim_after += 1
        ref_seq = ref_seq[:-1]
        alt_seq = alt_seq[:-1]
...
def iter_non_overlapping_variants(r_var_calls):
    def get_max_prob_allele_var(var_grp):
        ...
        (
            _,
            ref_lp,
            (var_pos, alt_lps, var_ref_seq, var_alt_seqs, _, _, _),
        ) = most_prob_var
...
def annotate_variants(r_start, ref_seq, r_var_calls, strand):
    ...
    for (
        ...
        var_alt_seqs,
        ref_lp,
    ) in iter_non_overlapping_variants(r_var_calls):
        ...
        if ref_lp >= max(alt_lps):
            ...
            var_seqs.append(
                ref_seq[prev_pos : var_pos - r_start] + read_alt_seq
            )
            var_quals.extend(
                ([VAR_MAP_MAX_QUAL] * prev_len)
                + (
...
def _get_variants_queue(
    ...
    def write_var_alignment(
        ...
        a = mapping.prepare_mapping(
            read_id,
            var_seq,
            flag=0 if strand == 1 else 16,
            ref_id=var_map_fp.get_tid(chrm),
            ref_st=r_st,
            qual=array("B", var_quals),
            map_qual=VAR_MAP_MAX_QUAL,
            tags=[("RG", VAR_MAP_RG_ID)],
            cigartuples=var_cigar,
        )
        var_map_fp.write(a)
    ...
    def store_var_call(
                ...
                var_out_text += (
                    "\n".join(
                        (
                            (
                                "\t".join(
                                    "{}" for _ in vars_db.text_field_names
                                )
                            ).format(
                                read_id,
                                chrm,
        ...
        if ref_out_info.do_output.var_pr_refs or vars_info.do_output.var_map:
            ...
            if (
                ref_out_info.do_output.var_pr_refs
                and mapping.read_passes_filters(
    ...
    try:
        ...
        if vars_info.do_output.text:
            vars_txt_fp = open(
                mh.get_megalodon_fn(vars_info.out_dir, mh.PR_VAR_TXT_NAME), "w"
            )
            vars_txt_fp.write("\t".join(vars_db.text_field_names) + "\n")
        if ref_out_info.do_output.var_pr_refs:
            pr_refs_fp = open(
                mh.get_megalodon_fn(vars_info.out_dir, mh.PR_REF_NAME), "w"
            )
        if vars_info.do_output.var_map:
            try:
...
class VarInfo:
    def check_vars_match_ref(
        self, vars_idx, contigs, aligner, num_contigs=5, num_sites_per_contig=50
    ):
            ...
            if len(set(aligner.seq_names).intersection(contigs)) == 0:
                raise mh.MegaError(
                    (
                        ...
                        + "\t{}\n\t\tFirst 3 variant file contigs:\t{}"
                    ).format(
                        ", ".join(aligner.seq_names[:3]), ", ".join(contigs[:3])
                    )
                )
    ...
    def annotate_context_seqs(
        ...
        for context_var, np_alt_seq in context_vars:
            ...
            prev_end = (
                context_var.stop - context_ref_start - context_rel_var_end
            )
    ...
    def iter_variant_combos_by_distance(variant, context_variants):
        ...
        def iter_alt_variant_seqs(variants):
            ...
            """
            vars_w_alts = [
                [(var, np_alt) for np_alt in var.np_alts] for var in variants
            ]
        ...
        for var_dist, dist_context_vars in sorted(dist_vars.items()):
            ...
            for n_dist_vars, n_used_vars in product(
                range(1, len(dist_context_vars) + 1), range(len(used_vars) + 1)
            ):
    ...
    def iter_context_variants(read_variants, context_max_dist):
        ...
        while next_var is not None:
            ...
            while (
                next_var is not None
                and next_var.start - context_max_dist <= curr_var.stop
            ):
                curr_vars.append(next_var)
                next_var = next_var_or_none()
            ...
            yield curr_var, [
                var
                for var in curr_vars
                if var.start >= curr_var.stop or curr_var.start >= var.stop
            ]
        ...
        while len(curr_vars) < curr_var_idx:
            ...
            yield curr_var, [
                var
                for var in curr_vars
                if var.start >= curr_var.end or curr_var.start >= var.end
            ]

    @staticmethod
    def add_indel_context_base(
        np_ref_seq, np_alt_seqs, var_start, read_ref_fwd_seq, read_ref_pos
    ):
        if np_ref_seq.shape[0] == 0 or any(
            np_alt.shape[0] == 0 for np_alt in np_alt_seqs
        ):
            upstrm_base = mh.ALPHABET[
                read_ref_fwd_seq[var_start - read_ref_pos.start - 1]
            ]
            var_start -= 1
        ...
        alt_seqs = tuple(
            (upstrm_base + mh.int_to_seq(np_alt) for np_alt in np_alt_seqs)
        )
        return ref_seq, alt_seqs, var_start, has_context_base

    def merge_variants(self, grouped_read_vars, read_ref_fwd_seq, read_ref_pos):
        ...
        for _, site_vars in sorted(grouped_read_vars.items()):
            ...
            for var in site_vars:
                if any(
                    (
                        len(var.np_alts[0]) == len(added_np_alt)
                        and np.all(var.np_alts[0] == added_np_alt)
                    )
            ...
            yield VARIANT_DATA(
                np_ref=site_vars[0].np_ref,
                np_alts=np_alts,
                id=site_var_ids,
                chrom=site_vars[0].chrom,
                start=site_vars[0].start,
                stop=site_vars[0].stop,
                ref=out_ref_seq,
                alts=out_alt_seqs,
                ref_start=out_start,
                strand=site_vars[0].strand,
                has_context_base=has_context_base,
            )
    ...
    def expand_ambig_variant(
        ...
        while (
            expand_pos < read_ref_fwd_seq.shape[0]
            and indel_seq[(expand_pos - expand_start) % indel_seq.shape[0]]
        ...
        np_ref_seq = np.concatenate(
            [expand_upstrm_seq, np_ref_seq, expand_dnstrm_seq]
        ).astype(np.uintp)
        np_alt_seq = np.concatenate(
            [expand_upstrm_seq, np_alt_seq, expand_dnstrm_seq]
        ).astype(np.uintp)
    ...
    def atomize_variants(self, read_vars, read_ref_fwd_seq, read_ref_pos):
        ...
        for var, np_alt_seq in (
            (var, nas) for var in read_vars for nas in var.np_alts
        ):
            # substitutions
            if np_alt_seq.shape[0] == var.np_ref.shape[0]:
                # convert all substitutions into single base substitutions
                for sub_offset, (np_ref_base, np_alt_base) in enumerate(
                    zip(var.np_ref, np_alt_seq)
                ):
                ...
                # skip large indels
                if (
                    np.abs(var.np_ref.shape[0] - np_alt_seq.shape[0])
                    > self.max_indel_size
                ):
                    continue
                ...
                grouped_read_vars[
                    (var_start, var_start + np_ref_seq.shape[0])
                ].append(
                    VARIANT_DATA(
                        np_ref=np_ref_seq,
                        np_alts=(np_alt_seq,),
    ...
    def parse_vars(self, fetch_res, read_ref_pos, read_ref_fwd_seq):
        for var in fetch_res:
            ...
            # are valid here
            if (
                var.stop + self.edge_buffer > read_ref_pos.end
                or var.start - self.edge_buffer < read_ref_pos.start
            ):
                continue
            # read strand if written (applicable for diff ctc signal mapping)
            strand = (
                int(var.info[mh.STRAND_FIELD_NAME])
                if mh.STRAND_FIELD_NAME in var.info
                else None
            )
            ...
            yield VARIANT_DATA(
                ...
                id=var.id,
                chrom=var.chrom,
                start=var.start + start_trim,
                stop=var.start + start_trim + np_ref_seq.shape[0],
                ref=var.ref,
                alts=var.alts,
                ref_start=var.start,
                strand=strand,
    ...
    def fetch_read_variants(self, read_ref_pos, read_ref_fwd_seq):
        try:
            fetch_res = self.variants_idx.fetch(
                read_ref_pos.chrm,
                read_ref_pos.start + self.edge_buffer,
    ...
    def iter_vars(
        ...
        read_ref_fwd_seq,
        max_contexts=16,
        context_max_dist=mh.CONTEXT_MAX_DIST,
    ):
        ...
        def extract_variant_contexts(variant, context_vars):
            ...
            var_context_bases = (
                ...
                if all(
                    ...
                    for np_alt in variant.np_alts
                )
                else self.indel_context
            )
            ...
            context_seqs = [
                (up_context_seq, dn_context_seq),
            ]

            if (
                context_max_dist == 0
                or max_contexts == 1
                or len(context_vars) == 0
            ):
                return (
                    context_ref_start,
                    context_read_start,
        ...
        for variant, context_variants in self.iter_context_variants(
            ...
            (
                ...
                context_read_end,
                np_context_seqs,
            ) = extract_variant_contexts(variant, context_variants)
    ...
    def calibrate_llr(self, llr, var_ref_seq, var_alt_seq):
        return self.calib_table.calibrate_llr(llr, var_ref_seq, var_alt_seq)
...
class Variant:
    ...
    def add_haploid_probs(self, probs, gts):
        ...
        self.add_sample_field(
            "PL",
            ",".join("{:.0f}" for _ in range(probs.shape[0])).format(
                *np.around(pl)
            ),
        )
...
class VcfWriter:
    ...
    def __init__(
        ...
        if version not in self.version_options:
            raise ValueError(
                "version must be one of {}".format(self.version_options)
            )
        self.version = version
        contig_mis = (
            []
            if ref_names_and_lens is None
            else [
...
class AggVars(mh.AbstractAggregationClass):
    ...
    def __init__(
        self,
        vars_db_fn,
        write_vcf_log_probs=False,
        load_uuid_index_in_memory=False,
    ...
    def compute_var_stats(
        self, var_loc, het_factors, call_mode=DIPLOID_MODE, valid_read_ids=None
    ):
        assert call_mode in (
            HAPLIOD_MODE,
            DIPLOID_MODE,
        ), "Invalid variant aggregation ploidy call mode: {}.".format(call_mode)
        ...
        if call_mode == DIPLOID_MODE:
            het_factor = (
                het_factors[0]
                if len(r0_stats.ref_seq) == 1 and len(r0_stats.alt_seq) == 1
                else het_factors[1]
            )
...
def get_whatshap_command(index_variant_fn, var_sort_fn):
    return (
        "See online documentation to obtain high-quality phased "
        + 'variants from outputs "{}" and "{}".'
    ).format(index_variant_fn, var_sort_fn)


def sort_variants(in_vcf_fn, out_vcf_fn):
    in_vcf_fp = pysam.VariantFile(in_vcf_fn)
    with pysam.VariantFile(
        out_vcf_fn, "w", header=in_vcf_fp.header
    ) as out_vcf_fp:
