...
DEFAULT_CONSTRAINED_HALF_BW = 10
...
GETTER_INFO = namedtuple(
    "GETTER_INFO", ("name", "do_output", "func", "args", "max_size")
)
GETTER_INFO.__new__.__defaults__ = (_MAX_QUEUE_SIZE,)
STATUS_INFO = namedtuple(
    "STATUS_INFO", ("suppress_prog_bar", "suppress_queues", "num_prog_errs")
)
INPUT_INFO = namedtuple(
    ...
    (
        ...
        "do_it_live",
        "num_read_enum_ts",
        "num_extract_sig_proc",
    ),
)
INPUT_INFO.__new__.__defaults__ = (
    False,
    DEFAULT_READ_ENUM_TS,
    DEFAULT_EXTRACT_SIG_PROC,
)
BASECALL_DO_OUTPUT = namedtuple(
    "BASECALL_DO_OUTPUT", ("any", "basecalls", "mod_basecalls")
)
BASECALL_INFO = namedtuple(
    "BASECALL_INFO",
    (
        "do_output",
        "out_dir",
...
REF_OUT_FILTER_PARAMS = namedtuple(
    "REF_OUT_FILTER_PARAMS", ("pct_idnt", "pct_cov", "min_len", "max_len")
)
REF_OUT_INFO = namedtuple(
    "ref_out_info",
    (
        "do_output",
        "filt_params",
...
MOD_DO_OUTPUT = namedtuple("MOD_DO_OUTPUT", ("db", "text", "mod_map", "any"))
...
SEQ_SUMM_INFO.__new__.__defaults__ = tuple(
    [
        "NA",
    ]
    * 12
...
def get_mean_q_score(read_q):
    ...
    return np.mean([q_val - PHRED_BASE for q_val in read_q.encode("ASCII")])
...
def get_supported_configs_message():
    configs = os.listdir(
        resolve_path(
            pkg_resources.resource_filename("megalodon", MODEL_DATA_DIR_NAME)
        )
    )
    if len(configs) == 0:
        return (
            "No guppy config calibration files found. Check that "
            + "megalodon installation is valid.\n"
        )
    out_msg = (
        "Megalodon support for guppy configs (basecalling and "
    ...
    for config in sorted(configs):
        config_files = os.listdir(
            resolve_path(
                pkg_resources.resource_filename(
                    "megalodon", os.path.join(MODEL_DATA_DIR_NAME, config)
                )
            )
        )
        out_msg += (
            "X" + " " * 18 if VAR_CALIBRATION_FN in config_files else " " * 19
        )
        out_msg += (
            "X" + " " * 18 if MOD_CALIBRATION_FN in config_files else " " * 19
        )
        out_msg += config + "\n"
    return out_msg
...
def parse_bed_scores(bed_fn):
    ...
    with open(bed_fn) as bed_fp:
        for line in bed_fp:
            ...
            bed_scores[(chrm, str_strand_to_int(strand))][int(pos)] = float(
                score
            )
...
def parse_beds(bed_fns, ignore_strand=False, show_prog_bar=True):
    ...
    for bed_fn in bed_fns:
        ...
        bed_iter = (
            tqdm(bed_fp, desc=bed_fn, smoothing=0) if show_prog_bar else bed_fp
        )
        for chrm, start, strand in bed_iter:
            store_strand = None if ignore_strand else str_strand_to_int(strand)
...
def parse_bed_methyls(
    bed_fns, strand_offset=None, show_prog_bar=True, valid_pos=None, limit=None
):
    ...
    for bed_fn in bed_fns:
        with open(bed_fn) as bed_fp:
            bed_iter = (
                tqdm(bed_fp, desc=bed_fn, smoothing=0)
                if show_prog_bar
                else bed_fp
            )
            for line in bed_iter:
                ...
                if valid_pos is not None and (
                    (chrm, store_strand) not in valid_pos
                    or start not in valid_pos[(chrm, store_strand)]
                ):
                    continue
...
def iter_bed_methyl_recs(bed_fn, batch_size=DEFAULT_BEDMETHYL_MINIBATCH):
        ...
        for line in bed_fp:
            ...
            if len(chrms) >= batch_size:
                ...
                yield from zip(
                    ...
                    strands,
                    np.around(
                        np.array(pct_mods, dtype=float) * cov / 100.0
                    ).astype(int),
                    cov,
                )
                chrms, poss, strands, cov, pct_mods = [], [], [], [], []
    if len(chrms) >= 0:
        ...
        yield from zip(
            ...
            map(int, poss),
            strands,
            np.around(np.array(pct_mods, dtype=float) * cov / 100.0).astype(
                int
            ),
            cov,
        )
...
def iter_merged_bedmethyl(rec_iters):
    ...
    curr_chrm = str(
        sorted(RefName(rec[0]) for rec in curr_recs if rec is not None)[0]
    )
    ...
    while any(rec is not None for rec in curr_recs):
        ...
        if all(rec[0] != curr_chrm for rec in curr_recs if rec is not None):
            ...
            if len(next_chrms) == 0:
                ...
                curr_chrm = str(sorted(RefName(chrm) for chrm in next_chrms)[0])
        curr_pos = min(
            rec[1]
...
def iter_bed_methyl_batches(
    bed_fn,
    strand_offset=None,
    batch_size=DEFAULT_BEDMETHYL_BATCH,
    valid_sites_fn=None,
    check_sorted=True,
):
            ...
            if chrm in used_chrms:
                raise MegaError(
                    (
                        ...
                        + '"{}" found after being observed previously.'
                    ).format(chrm)
                )
...
def parse_ground_truth_file(gt_data_fn, include_strand=True):
    ...
    with open(gt_data_fn) as fp:
        for line in fp:
            ...
            pos_key = (
                (chrm, str_strand_to_int(strand), int(pos))
                if include_strand
                else (chrm, int(pos))
            )
